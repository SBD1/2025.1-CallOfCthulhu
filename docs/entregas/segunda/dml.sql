...
-- 'pacífico' com acento é permitido pelo DDL atual
...
TRUNCATE TABLE public.pericias RESTART IDENTITY CASCADE; 
...
-- (1) Limpar e criar a tabela temporaria
...
-- (2) Inserir templos e locais e popular a temp_locais_ids em um bloco principal
WITH 
        ...
        locais_inseridos_cte AS ( 
                INSERT INTO public.local (descricao, tipo_local, status)
                VALUES 
                -- Salas (tipo_local = 'Sala', status eh NULL)
                ...
                -- Corredores (tipo_local = 'Corredor', status eh definido)
                ...
                RETURNING descricao, tipo_local, id 
        )
-- O INSERT final desta CTE preenche a tabela temporaria
INSERT INTO temp_locais_ids (descricao, tipo_local, id_local)
SELECT descricao, tipo_local, id FROM locais_inseridos_cte;


-- (3) Continuar com as outras inserções e atualizações dependentes,
-- AGORA ACESSANDO OS IDs DIRETAMENTE DE `public.local` ou `temp_locais_ids`

-- Inserindo o andar
INSERT INTO public.andares(descricao, id_templo, sala_inicial)
SELECT 'Primeiro andar do templo de Cthulhu', 
       (SELECT id FROM public.templos WHERE nome = 'Templo de Chutullu'), 
...
WHERE l.id IN (SELECT id_local FROM temp_locais_ids WHERE tipo_local = 'Sala'); 
...
-- Removendo a tabela temporaria DEPOIS DE TUDO que a usa
-- Esta instrucao deve ser a ULTIMA a acessar temp_locais_ids
