...
-- Monstro para o Estágio 2 da Missão
SELECT public.sp_criar_monstro(
    p_nome                  := 'Sombra Rastejante'::public.nome,
    p_descricao             := 'Uma escuridão que se move com intenção maligna, drenando a luz e a esperança.'::public.descricao,
    p_tipo                  := 'agressivo'::public.tipo_monstro,
    p_agressivo_defesa      := 12::SMALLINT,
    p_agressivo_vida        := 40::SMALLINT,
    p_agressivo_vida_total  := 40::SMALLINT,
    p_agressivo_catalisador := 'proximidade'::public.gatilho_agressividade,
    p_agressivo_poder       := 13::SMALLINT,
    p_agressivo_tipo        := 'fisico'::public.tipo_monstro_agressivo,
    p_agressivo_velocidade  := 8::SMALLINT,
    p_agressivo_loucura     := 10::SMALLINT,
    p_agressivo_pm          := 0::SMALLINT,
    p_agressivo_dano        := 25::public.dano
);

-- Monstro para o Estágio 3 da Missão
SELECT public.sp_criar_monstro(
    p_nome                  := 'Guardião da Cripta'::public.nome,
    p_descricao             := 'Uma estátua de pedra animada por uma força profana, seu único propósito é proteger os segredos da cripta.'::public.descricao,
    p_tipo                  := 'agressivo'::public.tipo_monstro,
    ...
    p_agressivo_tipo        := 'fisico'::public.tipo_monstro_agressivo, -- Campo obrigatório que estava faltando
    p_agressivo_velocidade  := 4::SMALLINT -- Campo obrigatório para monstros físicos
);
...
-- ===============================================
--       INSTÂNCIAS DE MONSTROS PARA O MODO HISTÓRIA
-- ===============================================

-- Monstro do Estágio 1 na sala circular
INSERT INTO public.instancias_monstros (id_monstro, vida, id_local, id_local_de_spawn)
SELECT
    ...
    (SELECT id FROM public.local WHERE descricao LIKE 'Um salão circular%'), -- Sala do Estágio 1
    (SELECT id FROM public.local WHERE descricao LIKE 'Um salão circular%');

-- Monstro do Estágio 2 na sala de entrada
INSERT INTO public.instancias_monstros (id_monstro, vida, id_local, id_local_de_spawn)
SELECT
    ...
    (SELECT id FROM public.local WHERE descricao LIKE 'O ar pesa%'), -- Sala do Estágio 2
    (SELECT id FROM public.local WHERE descricao LIKE 'O ar pesa%');

-- Monstro do Estágio 3 na câmara da abóbada
INSERT INTO public.instancias_monstros (id_monstro, vida, id_local, id_local_de_spawn)
SELECT
    (SELECT id FROM public.agressivos WHERE nome = 'Guardião da Cripta'),
    (SELECT vida_total FROM public.agressivos WHERE nome = 'Guardião da Cripta'),
    (SELECT id FROM public.local WHERE descricao LIKE 'Esta câmara é uma abóbada cavernosa%'), -- Sala do Estágio 3
