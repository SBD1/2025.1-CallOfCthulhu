...
WITH 
        ...
        locais_criados AS (
                INSERT INTO public.local (descricao)
        ...
        andar_criado AS (
                ...
                SELECT 'Primeiro andar do templo de Cthulhu', (SELECT id FROM templo_criado), (SELECT id FROM locais_criados WHERE descricao LIKE 'O ar pesa%')
                RETURNING id
        )
...
-- INSERT INTO public.corredores_salas_destino (id_local)
-- VALUES
...
WITH
  ...
  samuel AS (
    ...
    VALUES ('Samuel Carter', 'Doutor em Medicina', 'Arkham, MA', 'Boston, MA', 42, 'masculino', 10, 12, 12, 8, 15, 17, 13, 12, 7, 60, 12, 12, 14, (SELECT id FROM public.local WHERE descricao LIKE 'O ar pesa%'), (SELECT id FROM inv_samuel))
    RETURNING id
  ),
  inv_sarah AS ( INSERT INTO public.inventarios (tamanho) VALUES (28) RETURNING id ),
  sarah AS (
    INSERT INTO public.personagens_jogaveis (nome, ocupacao, residencia, local_nascimento, idade, sexo, forca, constituicao, poder, destreza, aparencia, tamanho, inteligencia, educacao, movimento, sanidade_atual, PM_base, PM_max, pontos_de_vida_atual, id_sala, id_inventario)
    VALUES ('Sarah Thompson', 'Arqueóloga', 'Boston, MA', 'Boston, MA', 35, 'feminino', 8, 10, 11, 14, 16, 15, 14, 13, 8, 55, 11, 11, 12, (SELECT id FROM public.local WHERE descricao LIKE 'Esta câmara é uma abóbada%'), (SELECT id FROM inv_sarah))
    RETURNING id
  )
...
WITH
  sabio AS (
    ...
    VALUES ('Velho Sábio', 'Guardião do Templo', 70, 'masculino', 'Templo das Sombras', 'Arkham', (SELECT id FROM public.local WHERE descricao LIKE 'O ar pesa%')) RETURNING id
  ),
  guarda AS (
    INSERT INTO public.npcs (nome, ocupacao, idade, sexo, residencia, local_nascimento, id_sala)
    VALUES ('Guarda do Templo', 'Protetor das Relíquias', 45, 'masculino', 'Templo das Sombras', 'Arkham', (SELECT id FROM public.local WHERE descricao LIKE 'Esta câmara é uma abóbada%')) RETURNING id
  ),
  sacerdotisa AS (
    INSERT INTO public.npcs (nome, ocupacao, idade, sexo, residencia, local_nascimento, id_sala)
    VALUES ('Sacerdotisa Sombria', 'Mestre dos Rituais', 50, 'feminino', 'Templo das Sombras', 'Arkham', (SELECT id FROM public.local WHERE descricao LIKE 'Um salão circular%')) RETURNING id
  ),
...
WITH
  ...
  instancia_adaga AS (
    ...
    SELECT 100, id, (SELECT id FROM public.local WHERE descricao LIKE 'Um salão circular%') FROM item_adaga RETURNING id
  ),
  instancia_monstro AS (
    INSERT INTO public.instancias_monstros (id_monstro, id_sala, id_instancia_de_item)
    SELECT (SELECT id FROM monstro_agressivo), (SELECT id FROM public.local WHERE descricao LIKE 'Um salão circular%'), id FROM instancia_adaga RETURNING id
  )
