...
Descrição: Refatoração da estrutura de localização: "Sala" e "Corredor" unificadas na tabela "Local".
            ...
            Alteração do tipo_local para INT e ajuste dos domínios relacionados.
Autor: Luiz Guilherme
...
ALTER TABLE public.personagens_jogaveis
DROP CONSTRAINT IF EXISTS fk_pj_salas; -- Antiga FK removida
ALTER TABLE public.personagens_jogaveis
DROP CONSTRAINT IF EXISTS fk_pj_corredores; -- Antiga FK removida
ALTER TABLE public.personagens_jogaveis
DROP CONSTRAINT IF EXISTS fk_pj_local; -- Nova FK, para dropar em caso de re-execução
...
DROP CONSTRAINT IF EXISTS fk_npcs_salas; -- Antiga FK removida
ALTER TABLE public.npcs
DROP CONSTRAINT IF EXISTS fk_npcs_corredores; -- Antiga FK removida
ALTER TABLE public.npcs
DROP CONSTRAINT IF EXISTS fk_npcs_local; -- Nova FK, para dropar em caso de re-execução
...
ALTER TABLE public.andares
DROP CONSTRAINT IF EXISTS fk_andares_salas; -- Antiga FK removida
ALTER TABLE public.andares
DROP CONSTRAINT IF EXISTS fk_andares_local_inicial; -- Nova FK, para dropar em caso de re-execução
...
DROP CONSTRAINT IF EXISTS fk_instancias_de_item_salas; -- Antiga FK removida
ALTER TABLE public.instancias_de_itens
DROP CONSTRAINT IF EXISTS fk_instancias_de_item_local; -- Nova FK, para dropar em caso de re-execução
...
ALTER TABLE public.instancias_monstros
DROP CONSTRAINT IF EXISTS fk_instancias_monstro_salas; -- Antiga FK removida
ALTER TABLE public.instancias_monstros
DROP CONSTRAINT IF EXISTS fk_instancias_monstro_corredores; -- Antiga FK removida
ALTER TABLE public.instancias_monstros
DROP CONSTRAINT IF EXISTS fk_instancias_monstro_local; -- Nova FK, para dropar em caso de re-execução
...
DROP TABLE IF EXISTS public.local; -- Tabela de Local única
...
DROP FUNCTION IF EXISTS public.gerar_id_local(); -- Para a função gerar_id_local
...
DROP DOMAIN IF EXISTS public.id_local; -- O domínio 'id_local' permanece como o ID principal para a tabela 'local'
...
CREATE DOMAIN public.tipo_monstro AS CHARACTER VARYING(9)
    CONSTRAINT tipo_monstro_check CHECK (
        (VALUE)::text = ANY (ARRAY[
            ...
            ('pacífico'::character VARYING)::text -- Corrigido para incluir o acento no DDL, para corresponder ao DML.
        ])
    );
...
-- Este domínio agora representa o ID para a entidade 'local'
...
CREATE TABLE public.local(
    ...
    descricao public.descricao NOT NULL,
    tipo_local int NOT NULL, -- Permanece INT, o DML será ajustado para isso
    status BOOLEAN,
    --FOREIGN KEYs para auto-relacionamento (conexões)
    sul public.id_local, 
    ...
    -- Restrições de chave estrangeira para as direções
    CONSTRAINT fk_local_sul FOREIGN KEY (sul) REFERENCES public.local(id),
    CONSTRAINT fk_local_norte FOREIGN KEY (norte) REFERENCES public.local(id),
    CONSTRAINT fk_local_leste FOREIGN KEY (leste) REFERENCES public.local(id),
    CONSTRAINT fk_local_oeste FOREIGN KEY (oeste) REFERENCES public.local(id),
    CONSTRAINT fk_local_cima FOREIGN KEY (cima) REFERENCES public.local(id),
    CONSTRAINT fk_local_baixo FOREIGN KEY (baixo) REFERENCES public.local(id)
);
...
CREATE TABLE public.agressivos(
    ...
    id_tipo_monstro INTEGER -- FK (Esta é uma FK lógica, não definida com constraint no DDL)
);
...
-- Tabela corredores_salas_destino foi removida do DDL.
...
-- Nenhuma nova restrição de CHECK adicionada explicitamente, confiando nas FKs e no design.
...
-- Nova FK para a tabela local (que substitui salas e corredores)
ALTER TABLE public.personagens_jogaveis 
...
-- Atualiza FK para a nova tabela 'local'
ALTER TABLE public.andares 
