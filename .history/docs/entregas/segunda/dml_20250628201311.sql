...
Versão: 1.1
Data: 28/06/2025
Descrição: Refatoração da estrutura de localização: "Sala" e "Corredor" unificadas na tabela "Local".
            Remoção de referências antigas e ajuste de FKs para a nova entidade "Local".
Autor: Luiz Guilherme
...
-- Removendo as FKs antigas de sala e corredor
ALTER TABLE public.personagens_jogaveis
...
-- Adicionando a FK para a nova tabela local (será criada no passo 3)
ALTER TABLE public.personagens_jogaveis
DROP CONSTRAINT IF EXISTS fk_pj_local;
...
-- Adicionando a FK para a nova tabela local (será criada no passo 3)
ALTER TABLE public.npcs
DROP CONSTRAINT IF EXISTS fk_npcs_local;
...
-- Ajustando a FK para sala_inicial que agora aponta para local
ALTER TABLE public.andares
DROP CONSTRAINT IF EXISTS fk_andares_salas;
ALTER TABLE public.andares
DROP CONSTRAINT IF EXISTS fk_andares_local_sala_inicial; -- Nova FK para local

-- da tabela corredores_salas_destino (esta tabela será removida)
...
-- Adicionando a FK para a nova tabela local (será criada no passo 3)
ALTER TABLE public.instancias_de_itens
DROP CONSTRAINT IF EXISTS fk_instancias_de_item_local;
...
-- Adicionando a FK para a nova tabela local (será criada no passo 3)
ALTER TABLE public.instancias_monstros
DROP CONSTRAINT IF EXISTS fk_instancias_monstro_local;
...
DROP TABLE IF EXISTS public.corredores_salas_destino; -- REMOVIDA
DROP TABLE IF EXISTS public.entregas_missoes; 
DROP TABLE IF EXISTS public.batalhas; 
DROP TABLE IF EXISTS public.conexoes_locais; -- Nova tabela para conexões de locais
...
DROP TABLE IF EXISTS public.salas; -- REMOVIDA
DROP TABLE IF EXISTS public.corredores; -- REMOVIDA
DROP TABLE IF EXISTS public.locais; -- Adicionada a nova tabela
...
-- REMOVENDO AS FUNÇÕES DE GERAÇÃO DE ID ANTIGAS PARA SALA E CORREDOR
DROP FUNCTION IF EXISTS public.gerar_id_sala();
DROP SEQUENCE IF EXISTS public.sala_id_seq;
DROP FUNCTION IF EXISTS public.gerar_id_corredor();
DROP SEQUENCE IF EXISTS public.corredor_id_seq;
DROP FUNCTION IF EXISTS public.gerar_id_local(); -- Para a nova função
...
-- DOMÍNIOS DE ID: Remover sala e corredor
...
DROP DOMAIN IF EXISTS public.id_sala; -- REMOVIDO
DROP DOMAIN IF EXISTS public.id_corredor; -- REMOVIDO
...
-- REMOVIDOS id_sala e id_corredor
-- CRIAÇÃO DO DOMÍNIO DAS SALAS (ANTIGO)
-- TODA SALA TEM ID QUE COMEÇA COM 0403
-- DROP DOMAIN public.id_sala AS public.id_localizacao;
--    CONSTRAINT id_sala_check CHECK (
--        VALUE BETWEEN 40300000 AND 40399999
-- );

-- CRIAÇÃO DO DOMÍNIO DOS CORREDORES (ANTIGO)
-- TODO CORREDOR TEM ID QUE COMEÇA COM 0404
-- DROP DOMAIN public.id_corredor AS public.id_localizacao;
...
-- GERA O ID DO PRÓXIMO LOCAL (SUBSTITUI SALA E CORREDOR)
CREATE SEQUENCE public.local_id_seq START WITH 1;

CREATE FUNCTION public.gerar_id_local()
RETURNS BIGINT AS $gerar_id_local$
BEGIN
    RETURN 40000000 + nextval('public.local_id_seq'); -- Usando o range geral de localizacao para a nova tabela 'locais'
END;
...
    id public.id_localizacao NOT NULL PRIMARY KEY DEFAULT public.gerar_id_local(),
    descricao public.descricao NOT NULL,
    tipo_local CHARACTER VARYING(8) NOT NULL, -- 'Sala' ou 'Corredor'
    -- Atributos específicos que eram de Sala ou Corredor, agora na única tabela Local
    status BOOLEAN, -- Originalmente de Corredor. NULL para Salas se não aplicável.
    -- proximo_local public.id_localizacao -- Removido daqui para a tabela de junção
    CONSTRAINT chk_tipo_local CHECK (tipo_local IN ('Sala', 'Corredor'))
);

-- Tabela para representar as conexões entre locais (antigo corredores_salas_destino)
CREATE TABLE public.conexoes_locais(
    id_local_origem public.id_localizacao NOT NULL,
    id_local_destino public.id_localizacao NOT NULL,
    direcao CHARACTER VARYING(10), -- Ex: 'Norte', 'Sul', 'Leste', 'Oeste', 'Cima', 'Baixo'
    bloqueado BOOLEAN DEFAULT FALSE,
    PRIMARY KEY (id_local_origem, id_local_destino)
);
...
    -- FOREIGN KEYS
    id_local public.id_localizacao, -- Aponta para a nova tabela 'locais'
...
    -- sala_inicial public.id_sala NOT NULL -- Antigo, agora aponta para local
    id_local_inicial public.id_localizacao NOT NULL -- Aponta para a nova tabela 'locais'
);

-- Tabelas 'salas' e 'corredores' foram REMOVIDAS.
...
-- REMOVIDA: public.corredores_salas_destino
...
/*
Essas próximas restrições garantem que o personagem ou NPC esteja em um local (que pode ser sala ou corredor).
A restrição chk_pj_local_exclusivo foi removida.
*/

-- Remover as restrições antigas que verificavam id_sala E id_corredor
ALTER TABLE public.personagens_jogaveis
DROP CONSTRAINT IF EXISTS chk_pj_local_exclusivo;
...
-- Adicionar nova restrição se necessário (ex: garantir que id_local não é NULL)
-- Se o id_local já for NOT NULL na definição da tabela, esta restrição é redundante.
-- ALTER TABLE public.personagens_jogaveis ADD CONSTRAINT chk_pj_local_not_null CHECK (id_local IS NOT NULL);
-- ALTER TABLE public.npcs ADD CONSTRAINT chk_npcs_local_not_null CHECK (id_local IS NOT NULL);
-- ALTER TABLE public.instancias_monstros ADD CONSTRAINT chk_monstros_local_not_null CHECK (id_local IS NOT NULL);
...
-- Nova FK para a tabela locais
ALTER TABLE public.personagens_jogaveis 
ADD CONSTRAINT fk_pj_local 
...
-- Atualiza FK para a nova tabela 'locais'
ALTER TABLE public.andares 
ADD CONSTRAINT fk_andares_local_inicial 
    FOREIGN KEY (id_local_inicial) 
    REFERENCES public.locais (id);

-- CONEXÕES LOCAIS (Substitui corredores_salas_destino)
ALTER TABLE public.conexoes_locais
ADD CONSTRAINT fk_conexoes_locais_origem
    FOREIGN KEY (id_local_origem)
    REFERENCES public.locais (id);

ALTER TABLE public.conexoes_locais
ADD CONSTRAINT fk_conexoes_locais_destino
...
-- Nova FK para a tabela locais
ALTER TABLE public.instancias_monstros 
ADD CONSTRAINT fk_instancias_monstro_local 
    FOREIGN KEY (id_local) 
    REFERENCES public.locais (id);
...
-- Nova FK para a tabela locais
ALTER TABLE public.instancias_de_itens 
ADD CONSTRAINT fk_instancias_de_item_local 
    FOREIGN KEY (id_local) 
    REFERENCES public.locais (id);  
